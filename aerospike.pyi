...
class Client:
    ...
    def put(policy: WritePolicy, key: Key, bins: dict[str, PythonValue]) -> None: ...
    ...
    def add(policy: WritePolicy, key: Key, bins: dict[str, PythonValue]) -> None: ...
    ...
    def append(policy: WritePolicy, key: Key, bins: dict[str, PythonValue]) -> None: ...

    # Prepend bin string values to existing record bin values. The policy specifies the
    # transaction timeout, record expiration and how the transaction is handled when the record
    # already exists. This call only works for string values.
    def prepend(policy: WritePolicy, key: Key, bins: dict[str, PythonValue]) -> None: ...
    ...
    def delete(policy: WritePolicy, key: Key) -> bool: ...

    # Reset record's time to expiration using the policy's expiration. Fail if the record does
    # not exist.
    def touch(policy: WritePolicy, key: Key) -> None: ...

    # Determine if a record key exists. The policy can be used to specify timeouts.
    def exists(policy: WritePolicy, key: Key) -> bool: ...

    # Removes all records in the specified namespace/set efficiently.
    def truncate(namespace: str, set_name: str, before_nanos: int | None) -> None: ...
    ...
    def create_index(namespace: str, set_name: str, bin_name: str, index_name: str, index_type: IndexType, cit: CollectionIndexType | None) -> None: ...

    def drop_index(namespace: str, set_name: str, index_name: str) -> None: ...
    ...
    # read in series.
    def scan(policy: ScanPolicy, namespace: str, set_name: str, bins: list[str] | None) -> Recordset: ...
...
def new_client(policy: ClientPolicy, hosts: str) -> Client: ...
