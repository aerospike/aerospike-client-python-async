...
class TestStatementAggregateFunction(TestFixtureConnection):
    """Test Statement.set_aggregate_function method."""

    async def test_set_aggregate_function_with_args(self, client):
        """Test setting aggregate function with arguments."""
        stmt = Statement("test", "test")
        stmt.set_aggregate_function("mypackage", "myfunction", ["arg1", 123])

        # Verify statement was configured (no error raised)
        assert stmt is not None

    async def test_set_aggregate_function_without_args(self, client):
        """Test setting aggregate function without arguments."""
        stmt = Statement("test", "test")
    ...
    async def test_set_aggregate_function_various_arg_types(self, client):
        """Test setting aggregate function with various argument types."""
        stmt = Statement("test", "test")

        # Test with string arg
        stmt.set_aggregate_function("pkg", "func", ["string_arg"])

        # Test with numeric args
