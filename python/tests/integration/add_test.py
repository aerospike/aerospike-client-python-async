...
class TestAdd(TestFixtureInsertRecord):
    ...
    async def test_add_default_policy(self, client, key):
        """add() succeeds without an explicit policy (uses default)."""
    ...
    async def test_add_with_policy(self, client, key):
        ...
        wp = WritePolicy()
        retval = await client.add(key, {"year": 1}, policy=wp)
        assert retval is None

    async def test_add_unsupported_bin_type(self, client, key):
        """Test add operation with unsupported bin type raises BinTypeError."""
        with pytest.raises(BinTypeError) as exc_info:
            await client.add(key, {"brand": 1}, policy=WritePolicy())
