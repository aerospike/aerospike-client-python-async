...
"""Integration tests for string filter expressions (server 8.2.0+).
...
async def string_client_820(aerospike_host, supports_string_operations, use_services_alternate):
    """8.2.0+ client. See ``operate_string_test.py:string_client_820``.

    Single-host model: default ``AEROSPIKE_HOST`` + ``supports_string_operations``
    capability gate (server >= 8.2.0)."""
...
class TestStringReadExpressions:
    ...
    async def test_strlen_byte_length_blob(self, string_client_820):
        key = _key("read_basics")
        await string_client_820.put(key, {"s": "héllo"}, policy=WritePolicy())
        assert await _eval_exp(string_client_820, key, Exp.string_strlen(Exp.string_bin("s"))) == 5
        assert (
            await _eval_exp(string_client_820, key, Exp.string_byte_length(Exp.string_bin("s")))
        ) == 6
        assert (
            await _eval_exp(string_client_820, key, Exp.string_to_blob(Exp.string_bin("s")))
        ) == "héllo".encode("utf-8")

    async def test_substr_offset_form_and_range_form(self, string_client_820):
        key = _key("substr")
        await string_client_820.put(key, {"s": "hello world"}, policy=WritePolicy())
        # offset-to-end
        out = await _eval_exp(
            string_client_820, key, Exp.string_substr(Exp.int_val(6), Exp.string_bin("s"))
        )
        assert out == "world"
    ...
    async def test_char_at_and_find(self, string_client_820):
        key = _key("char_find")
        await string_client_820.put(key, {"s": "hello"}, policy=WritePolicy())
        assert (
            await _eval_exp(
                string_client_820, key, Exp.string_char_at(Exp.int_val(1), Exp.string_bin("s"))
            )
        ) == "e"
        # find first match
        assert (
            await _eval_exp(
                string_client_820, key, Exp.string_find(Exp.string_val("ll"), Exp.string_bin("s"))
            )
        ) == 2
        # find N-th match
        await string_client_820.put(key, {"s": "ab ab ab"}, policy=WritePolicy())
        assert (
            await _eval_exp(
                string_client_820,
                key,
    ...
    async def test_contains_starts_ends_decode_as_bool(self, string_client_820):
        ...
        for exp_builder, expected in [
            ...
            out = await _eval_exp(string_client_820, key, exp_builder())
            assert out is expected
            assert isinstance(out, bool), f"expected bool, got {type(out).__name__}"

    async def test_is_numeric_default_and_int_only(self, string_client_820):
        key = _key("numeric")
        await string_client_820.put(key, {"d": "3.14", "i": "42"}, policy=WritePolicy())
        # ANY accepts float-formatted.
        assert (
            await _eval_exp(string_client_820, key, Exp.string_is_numeric(Exp.string_bin("d")))
        ) is True
    ...
    async def test_is_upper_is_lower(self, string_client_820):
        key = _key("case")
        await string_client_820.put(key, {"u": "HELLO", "l": "hello"}, policy=WritePolicy())
        assert (
            await _eval_exp(string_client_820, key, Exp.string_is_upper(Exp.string_bin("u")))
        ) is True
        assert (
            await _eval_exp(string_client_820, key, Exp.string_is_lower(Exp.string_bin("l")))
        ) is True

    async def test_classifiers_observe_a_chained_case_op(self, string_client_820):
    ...
    async def test_split_with_and_without_separator(self, string_client_820):
        key = _key("split")
        await string_client_820.put(key, {"s": "a,b,c"}, policy=WritePolicy())
        assert (
            await _eval_exp(
                string_client_820,
                key,
        ...
        await string_client_820.put(key, {"s": "hi"}, policy=WritePolicy())
        assert (
            await _eval_exp(string_client_820, key, Exp.string_split(Exp.string_bin("s")))
        ) == ["h", "i"]

    async def test_regex_compare_bin_source_with_case_insensitive(self, string_client_820):
...
class TestStringModifyExpressions:
    ...
    async def test_upper_returns_modified_value_does_not_persist(self, string_client_820):
        key = _key("upper_pure")
        await string_client_820.put(key, {"s": "hello"}, policy=WritePolicy())
        # Returns "HELLO" but the stored bin stays lowercase.
        out = await _eval_exp(
            string_client_820, key, Exp.string_upper(0, Exp.string_bin("s"))
        )
        assert out == "HELLO"
        rec = await string_client_820.get(key)
        assert rec.bins.get("s") == "hello"

    async def test_chained_modify_into_read_expression(self, string_client_820):
    ...
    async def test_replace_and_replace_all(self, string_client_820):
    ...
    async def test_insert_and_overwrite_splice_at_a_codepoint_index(self, string_client_820):
        ...
        await string_client_820.put(key, {"s": "abcdef"}, policy=WritePolicy())
        out = await _eval_exp(
            string_client_820,
            key,
    ...
    async def test_concat_appends_a_list_in_order(self, string_client_820):
        key = _key("concat_exp")
        await string_client_820.put(key, {"s": "hello"}, policy=WritePolicy())
        out = await _eval_exp(
            string_client_820,
    ...
    async def test_append_and_prepend_join_onto_the_subject(self, string_client_820):
        ...
        rec = await string_client_820.get(key, policy=None)
        assert rec.bins["s"] == "hello"

    async def test_snip_removes_a_half_open_range(self, string_client_820):
        key = _key("snip_exp")
        await string_client_820.put(key, {"s": "hello world"}, policy=WritePolicy())
        out = await _eval_exp(
            string_client_820,
    ...
    async def test_snip_from_truncates_to_end(self, string_client_820):
    ...
    async def test_pad_end_appends_to_the_target_length(self, string_client_820):
        key = _key("pad_end_exp")
        await string_client_820.put(key, {"s": "ab"}, policy=WritePolicy())
        out = await _eval_exp(
            string_client_820,
            key,
    ...
    async def test_regex_replace_global_flag(self, string_client_820):
    ...
    async def test_pad_repeat(self, string_client_820):
...
class TestToStringExpression:
    ...
    async def test_int_to_string_via_expression(self, string_client_820):
        key = _key("ts_exp_int")
        await string_client_820.put(key, {"n": 42}, policy=WritePolicy())
        out = await _eval_exp(
            string_client_820, key, Exp.to_string(Exp.int_bin("n"))
        )
        assert out == "42"
...
class TestStringExpNestedSource:
    ...
    async def test_upper_on_string_at_map_key(self, string_client_820):
        key = _key("nested_map")
        await string_client_820.put(
